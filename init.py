...
import base64  # encoding/decoding for plots
...
# pie chart to view the history consumption
...
# get information from the database for public search
...
def search():
    if request.method == 'POST':
        ...
        if (not flag1 and not flag2 and not flag3 and not flag4 and not flag5):
            ...
            sub1 = " departure_airport IN (SELECT airport_name FROM airport WHERE airport_city=\'{}\') ".format(
                departure_city)
            sub2 = ' departure_airport = \"{}\" '.format(departure_airport)
            sub3 = " arrival_airport IN (SELECT airport_name FROM airport WHERE airport_city=\'{}\') ".format(
                arrival_city)
            ...
            for flight in search:
                ...
                total_query = "SELECT seats FROM airplane WHERE airline_name = \'{}\' AND airplane_id = \'{}\'".format(
                    airline, airplane_id)
                cursor.execute(total_query)
                # find available seats for the flight (find the corresponding airplane to get the seats information)
                total = cursor.fetchone()[0]  
                sold_query = "SELECT COUNT(*) FROM ticket WHERE airline_name = \'{}\' AND  flight_num = \'{}\'".format(
                    airline, flight_num)
                cursor.execute(sold_query)
                # find the sold num of tickets for that flight
                sold = cursor.fetchone()[0]  
                available = int(total) - int(sold)
                available_seats.append(available)  
...
# route for checking flight status
@app.route('/flight_status')
def flight_status():
    return render_template('flight_status.html')


# search the flight and check the status
...
def check_status():
    if request.method == 'POST':
        ...
        if (not arrival_date and not departure_date and not flight_number): # at least 1 field should be specified
            error = "At least 1 field should be specified!"
            ...
            query = "SELECT airline_name, flight_num, departure_time, arrival_time, status FROM flight WHERE " + " AND ".join(
                merged_sub)
            cursor = conn.cursor()
            cursor.execute(query)
...
# route for booking agent register
...
# route for airline staff register
...
def staffRegister():
    ...
    airline_list = [row[0] for row in data]

    return render_template('staff_register.html', airline_list=airline_list)
        ...
        if (not flag1 and not flag2 and not flag3 and not flag4 and not flag5):
            ...
            for flight in search:
                ...
                total_query = "SELECT seats FROM airplane WHERE airline_name = \'{}\' AND airplane_id = \'{}\'".format(
                    airline, airplane_id)
                cursor.execute(total_query)
                total = cursor.fetchone()[0]  # total seats
                sold_query = "SELECT COUNT(*) FROM ticket WHERE airline_name = \'{}\' AND  flight_num = \'{}\'".format(
                    airline, flight_num)
                cursor.execute(sold_query)
                sold = cursor.fetchone()[0]  # sold seats
                available = int(total) - int(sold)
                available_seats.append(available)  
            for i in range(len(search)): 
                original = list(search[i])
...
def customer_purchase():
    if request.method == "POST":
        ...
        multiple_purchase_query = "SELECT * FROM purchases NATURAL JOIN ticket WHERE customer_email=\'{}\' AND airline_name=\'{}\' AND flight_num=\'{}\'".format(
            username, airline, flight_num)
        cursor.execute(multiple_purchase_query)
        multiple_purchase = cursor.fetchone()
        if (multiple_purchase):
            warning = "Warning: Your have previously already purchased ticket for flight \"{}-{}\", no multiple purchases allowed!".format(
                airline, flight_num)
            flash(warning)
        ...
        # booking_agent = NULL, purchase date is now()
        purchase_record_query = "INSERT INTO purchases VALUES (\'{}\', \'{}\', NULL , DATE(NOW()) )".format(new_id,
                                                                                                            username)
...
# show customer spending during the past 6 months
@app.route('/customer_spending', methods=['GET', 'POST'])
def customer_spending():
    username = session['username']
    cursor = conn.cursor()
    last_year_query = "SELECT SUM(price) FROM purchases NATURAL JOIN ticket NATURAL JOIN flight  WHERE customer_email= \'{}\' AND purchase_date BETWEEN date_sub(DATE(NOW()), interval 1 year) AND DATE(NOW())".format(
        username)
    ...
    # generate the past 6 months' monthly spending
    ...
    month_query_list = [month_query.format("0", "0", username, "0", "0"),
                        ...
                        month_query.format("5", "5", username, "5",
                                           "5")] 
    month_index = []
    ...
    cursor.close()
    # generate the return to make the url for plot
    img = BytesIO()
...
def agent_view_flight():
    ...
    # all upcoming flights the agent ordered for customers
...
def agent_customize_view():
    if request.method == 'POST':
        ...
        if (not flag1 and not flag2 and not flag3 and not flag4 and not flag5):
            error = "At least 1 field should be specified!"
            return render_template('static/agent_customize_view.html', error=error)
        else:
            sub1 = " f.departure_airport IN (SELECT airport_name FROM airport WHERE airport_city=\'{}\') ".format(
                departure_city)
            sub2 = " f.departure_airport = \"{}\" ".format(departure_airport)
            sub3 = " f.arrival_airport IN (SELECT airport_name FROM airport WHERE airport_city=\'{}\') ".format(
                arrival_city)
            sub4 = " f.arrival_airport = \"{}\" ".format(arrival_airport)
            if (flag_start and flag_end):
                sub5 = "( DATE(f.departure_time) BETWEEN DATE(\'{}\') AND DATE(\'{}\') )".format(starting_date,
                                                                                                 ending_date)
            ...
            search = cursor.fetchall()
            cursor.close()
        return render_template('static/agent_customize_view.html', search=search)
...
# agent search and view customer info
...
# agent purchase ticket for customer
...
def agent_view_commission():
    ...
    query = "SELECT COALESCE(0.1 * SUM(f.price), 0) " + \
            "FROM purchases p, ticket t, flight f " + \
            "WHERE p.booking_agent_id = \'{}\' AND p.ticket_id = t.ticket_id AND " + \
            "t.airline_name = f.airline_name AND t.flight_num = f.flight_num AND " + \
...
# customize: self-determine the starting date and ending date
...
def agent_customize_commission():
    if request.method == 'POST':
        ...
        return render_template('static/agent_customize_commission.html', commission=commission,
                               num_ticket=num_ticket)


# search and view top customer
...
def agent_top_customer():
    ...
    cursor.execute(query.format(agent_id[0]))
    customer = []
    # search through the results to fetch <= top 5 customers if exist
...
def staff_search_result():
    if request.method == 'POST':
        ...
        if (not flag1 and not flag2 and not flag3 and not flag4 and not flag5):
            ...
            sub4 = " arrival_airport =\"{}\" ".format(arrival_airport)
            if (flag_start and flag_end):
                sub5 = "( DATE(departure_time) BETWEEN DATE(\'{}\') AND DATE(\'{}\') )".format(starting_date,
                                                                                               ending_date)
            elif (flag_start and not flag_end):
                ...
                sub5 = "empty query"
            merged_sub = list(
                filter(None, [sub0, flag1 * sub1, flag2 * sub2, flag3 * sub3, flag4 * sub4, flag5 * sub5]))
...
# staff create a flight
@app.route('/home/staff_create_flight')
def staff_create_flight():
    username = session['username']

    cursor = conn.cursor()
    # select the airline that the staff is in
    query = "SELECT airline_name FROM airline_staff WHERE username = \'{}\'"
    cursor.execute(query.format(username))
    airline = cursor.fetchone()
    # select the upcoming flights of this airline
...
@app.route('/home/StaffCreateAuth', methods=['POST'])
def StaffCreateAuth():
    username = session['username']
    identity = session['identity']
    ...
    # data1 == True, then flight exists, return error
    if (data1):
        flash("This flight already exists!")
        return redirect(url_for('staff_create_flight'))
    # error: two airports cannot be the same
    if departure_airport == arrival_airport:
        flash("Departure and arrival airport cannot be the same!")
        return redirect(url_for('staff_create_flight'))
    # error: departure time must be earlier than arrival time
    if departure_time >= arrival_time:
        flash("Departure time must be earlier than arrival time!")
        return redirect(url_for('staff_create_flight'))

    ins = "INSERT INTO flight VALUES(\'{}\', \'{}\', \"{}\", \'{}\', \"{}\", \'{}\', \'{}\', \'{}\', \'{}\')".format(
...
def StaffAddPlaneAuth():
    ...
    # assign an airplane_id to the added plane (prev+1)
    cursor = conn.cursor()
...
def StaffAddAirportAuth():
    ...
    city = request.form['city']
    # check whether the name of the airport is the same with the existing airports
    cursor = conn.cursor()
    query = "SELECT EXISTS(SELECT * FROM airport WHERE airport_name = \"{}\"); "
    cursor.execute(query.format(airport))
    flag = cursor.fetchone()
    if flag[0] == 1:  # error: name already exist
...
# staff view top agent for their airline
...
# staff view most frequent customer
...
def staff_top_destination():
    ...
    airline = cursor.fetchone()
    cursor.close()
    # find top destinations in the past 3 months
    ...
    destination = []
    # serach through the results to fetch <= top 3 flights if exist
    ...
    destination = []
    # search through the results to fetch <= top 3 flights if exist
    dest = cursor.fetchone()
    count = 1
    while dest and count <= 3:
        destination.append(list(dest))
...
# staff compare the revenue of the agents
...
def staff_revenue_comparison():
    ...
    # last month non-agent revenue
    non_agent_last_month_query = "SELECT SUM(price) FROM flight NATURAL JOIN purchases NATURAL JOIN ticket WHERE airline_name=\'{}\' AND booking_agent_id is NULL AND (purchase_date BETWEEN DATE_SUB(DATE(NOW()), interval 30 DAY) AND DATE(NOW()) )".format(
        airline)
    cursor = conn.cursor()
    ...
    agent_last_month_query = "SELECT SUM(price) FROM flight NATURAL JOIN purchases NATURAL JOIN ticket WHERE airline_name=\'{}\' AND booking_agent_id is NOT NULL AND (purchase_date BETWEEN DATE_SUB(DATE(NOW()), interval 30 DAY) AND DATE(NOW()) )".format(
        airline)
    cursor = conn.cursor()
    ...
    # last year non-agent revenue
    non_agent_last_year_query = "SELECT SUM(price) FROM flight NATURAL JOIN purchases NATURAL JOIN ticket WHERE airline_name=\'{}\' AND booking_agent_id is NULL AND (purchase_date BETWEEN DATE_SUB(DATE(NOW()), interval 365 DAY) AND DATE(NOW()) )".format(
        airline)
    cursor = conn.cursor()
    ...
    agent_last_year_query = "SELECT SUM(price) FROM flight NATURAL JOIN purchases NATURAL JOIN ticket WHERE airline_name=\'{}\' AND booking_agent_id is NOT NULL AND (purchase_date BETWEEN DATE_SUB(DATE(NOW()), interval 365 DAY) AND DATE(NOW()) )".format(
        airline)
    cursor = conn.cursor()
    ...
    # get the date of last month and last year
    ...
    # show pie chart for comparison
    img = BytesIO()
    fig, (ax1, ax2) = plt.subplots(2, figsize=(10, 10))
    fig.suptitle('Direct v.s. Indirect Revenue earned')
    ax1.set_title('last month')
    ax1.pie([non_agent_last_month, agent_last_month], labels=['Direct', 'Indirect'],
            autopct=make_autopct([non_agent_last_month, agent_last_month]))
    ax2.set_title('last year')
    ax2.pie([non_agent_last_year, agent_last_year], labels=['Direct', 'Indirect'],
            autopct=make_autopct([non_agent_last_year, agent_last_year]))
    fig.legend(loc='center right')
    fig.savefig(img, format='png')
    img.seek(0)
    plot_url = base64.b64encode(img.getvalue()).decode('utf8')

    return render_template('staff_revenue_comparison.html', airline=airline, today_date=today_date,
                           last_month_date=last_month_date, last_year_date=last_year_date,
                           non_agent_last_month=non_agent_last_month, agent_last_month=agent_last_month,
...
def staff_view_report():
    # extract which airline that the staff is in
    username = session['username']
    airline_query = "SELECT airline_name FROM airline_staff WHERE username=\'{}\'".format(username)
    cursor = conn.cursor()
    cursor.execute(airline_query)
    ...
    # date of last month ticket sold
    last_month_ticket_query = "SELECT COUNT(*) FROM purchases NATURAL JOIN flight NATURAL JOIN ticket WHERE airline_name=\'{}\' AND DATE(purchase_date) BETWEEN \'{}\' AND \'{}\'".format(
        airline, last_month_date, today_date)
    cursor = conn.cursor()
    ...
    # date of last year ticket sold
    last_year_ticket_query = "SELECT COUNT(*) FROM purchases NATURAL JOIN flight NATURAL JOIN ticket WHERE airline_name=\'{}\' AND DATE(purchase_date) BETWEEN \'{}\' AND \'{}\'".format(
        airline, last_year_date, today_date)
    cursor = conn.cursor()
    ...
    # show the past year ticket sales report in a bar chart
    ...
    month_query_list = [month_query.format("0", "0", airline, "0", "0"),
                        month_query.format("1", "1", airline, "1", "1"),
                        month_query.format("2", "2", airline, "2", "2"),
                        month_query.format("3", "3", airline, "3", "3"),
                        month_query.format("4", "4", airline, "4", "4"),
                        month_query.format("5", "5", airline, "5", "5"),
                        month_query.format("6", "6", airline, "6", "6"),
                        month_query.format("7", "7", airline, "7", "7"),
                        month_query.format("8", "8", airline, "8", "8"),
                        month_query.format("9", "9", airline, "9", "9"),
                        month_query.format("10", "10", airline, "10", "10"),
                        month_query.format("11", "11", airline, "11", "11"),
                        month_query.format("12", "12", airline, "12", "12")]
    # generate the past year's ticket sold counting
    month_index = []
    ...
    return render_template("staff_view_report.html", airline=airline, today_date=today_date,
                           last_month_date=last_month_date, last_year_date=last_year_date,
                           last_month_ticket=last_month_ticket, last_year_ticket=last_year_ticket, plot_url=plot_url)


# staff customize search: self-determine the starting date and the ending date
...
def staff_customize_view_report():
    ...
    if ((not start) or (not end)):  # both start/end month for ticket sale search must be specified
        ...
        cursor.close()
        # query for each month's ticket sale
        ...
        month_query_list = [month_query.format(end, i, end, i, airline, end, i, end, i) for i in
                            range(month_gap + 1)] 
        month_index = []
        ...
        return render_template('staff_customize_view_report.html', start_year=start_year, start_month=start_month,
                               end_year=end_year, end_month=end_month, airline=airline, plot_url=plot_url)
